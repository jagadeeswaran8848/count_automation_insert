...
def connect_to_mysql():
    try:
        connection = mysql.connector.connect(
            ...
            password="root8848",  # Replace with your MySQL password
...
def execute_query(query, connection):
    ...
    try:
        ...
        connection.commit()
        st.success(f"Query executed successfully: {query}")
    except Error as e:
...
def main():
    ...
    # Initialize queries list outside the button logic
    ...
    if st.button("Generate Insert Queries"):
        st.session_state.queries.clear()  # Clear previous queries if the button is clicked again
            ...
            if isinstance(inputs, dict):
                if 'min' in inputs['inc']:  # Range inputs (Employees, Turnover, Company Age)
                    ...
                    if column in ["Employees", "Turnover", "Company Age"]:
                        ...
                        st.session_state.queries.append(f"-- Insert queries for {column_name} (Range)")
                        for range_type in ['inc', 'exc']:
                            ...
                            if isinstance(range_values, dict):  # This is for min/max range inputs
                                ...
                                if min_value is not None and min_value != 0:
                                    ...
                                    query = f"INSERT INTO dev.count_spec_template_updated_sample (`values`, `attribute`, `type`, `order_name`, `load_date`, `status`, `completed_status`)\n VALUES \n('{min_value}', '{column_name}', '{range_type.upper()}', '{order_name}', current_date(), NULL, NULL);"
                                    st.session_state.queries.append(query)
                                if max_value is not None and max_value != 0:
                                    # Generate a separate query for the MAX value
                                    query = f"INSERT INTO dev.count_spec_template_updated_sample (`values`, `attribute`, `type`, `order_name`, `load_date`, `status`, `completed_status`)\n VALUES \n('{max_value}', '{column_name}', '{range_type.upper()}', '{order_name}', current_date(), NULL, NULL);"
                                    st.session_state.queries.append(query)
                else:  # Regular inputs (Sub Industry, etc.)
                    # Add comment for the group
                    st.session_state.queries.append(f"-- Insert queries for {column_name}")
                        ...
                        if values:
                            for value in values:
                                ...
                                if range_type == 'inc':
                                    ...
                                    continue
                                query = f"INSERT INTO dev.count_spec_template_updated_sample (`values`, `attribute`, `type`, `order_name`, `load_date`, `status`, `completed_status`)\n VALUES \n('{value}', '{column_name}', '{type_value}', '{order_name}', current_date(), NULL, NULL);"
                                st.session_state.queries.append(query)
    ...
    if st.button("Execute Insert Queries"):
        if st.session_state.queries:
            ...
            st.error("Please generate the queries first.")
